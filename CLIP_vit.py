...
class TextEncoder(nn.Module):
    ...
    def forward(self, cap_tokens, cap_pad_mask, cap_causal_mask):
        cap_embs = self.caption_embeddings(cap_tokens)
        encoded_text = self.encoder(cap_embs, cap_pad_mask, cap_causal_mask)
        return encoded_text
...
class CLIP(nn.Module):
    def __init__(self, text_encoder, image_encoder, d_model, eos_token):
        ...
        self.eos_token = eos_token # used to get eos indices in cap_tokens
        self.eos_proj = nn.Linear(d_model, d_model, bias=False) # this is needed since the d_model of text_encoder can be different from d_model of image_encoder (not the case in this implementation though)
    ...
    #     return scores
    def encode(self, imgs, cap_tokens, cap_pad_mask, cap_causal_mask):
        encoded_txt = self.text_encoder(cap_tokens, cap_pad_mask, cap_causal_mask) # encoded_txt.shape: [batch_size, max_seq_len, d_model]
        encoded_img = self.image_encoder.encode(imgs) # encoded_txt.shape: [batch_size, seq_len_vit, d_model]
        return encoded_img, encoded_txt
    def forward(self, imgs, cap_tokens, cap_pad_mask, cap_causal_mask):
        # get eos indices in cap_tokens
        eos_idx = torch.where(cap_tokens == eos_token) # eos_idx is tupe of shape (batch_size, 2)
        # encode imgs and captions
        encoded_img, encoded_txt = self.encode(imgs, cap_tokens, cap_pad_mask, cap_causal_mask)
        # extract cls index as representation for encoded_img
        encoded_img = encoded_img[:, 0] # encoded_img.shape: [batch_size, d_model]
        # extract eos index as representation for encoded_txt
        encoded_txt = encoded_txt[eos_idx]
        # project encoded_txt to d_model of encoded_img (this is useful when d_model of image_encoder is different from d_model of text_encoder)
        encoded_txt = self.eos_proj(encoded_txt) # encoded_txt.shape: [batch_size, d_model]
    ...
    def predict(self, imgs, cap_tokens, cap_pad_mask, cap_causal_mask, threshold=0.9):
        scores = self.forward(imgs, cap_tokens, cap_pad_mask, cap_causal_mask)
        scores = scores.trace()
...
def init_text_encoder(vocab_size, max_seq_len, d_model, d_k, d_v, n_heads, n_layers, d_ff, dropout, device):
    ...
    model = TextEncoder(encoder, caption_embeddings)
...
def process_batch(minibatch, model, spm_processor, sos_token, eos_token, pad_token, max_seq_len, img_size, device):
    ...
    cap_causal_mask = subsequent_mask(tokenized_captions.shape).to(device)
    return augmented_imgs, tokenized_captions, cap_pad_mask, cap_causal_mask

# function to calculate loss by forward propping through the model
def calculate_loss(model, imgs, cap_embs, cap_pad_mask, cap_causal_mask, batch_size, device):
    # feed embeddings to transformer decoder
    scores = model(imgs, cap_embs, cap_pad_mask, cap_causal_mask) # scores.shape: [batch_size, batch_size]
    # targets
...
def calculate_test_accuracy(test_trials, model, test_data):
    ...
    for ep in range(test_trials):
        ...
        threshold = 0.9
        # process batch to get batch of imgs_files and captions to get imgs and tokenized captions
        imgs, cap_tokens, cap_pad_mask, cap_causal_mask = process_batch(minibatch, model, spm_processor, sos_token, eos_token, pad_token, max_seq_len, img_size, device)
        # feed to transformer decoder
        pred_labels = model.predict(imgs, cap_tokens, cap_pad_mask, cap_causal_mask, threshold) # pred_labels.shape: [batch_size]
        # accuracy
...
def save_checkpoint(checkpoint_path, model, optimizer, lr_scheduler):
...
if __name__ == '__main__':
    ...
    seq_len_vit = (img_size // patch_size) ** 2
    out_dim = 10 # dummy param for this implementation (for vit_init)

    # hyperparams for transformer (both ViT and text_encoder)
    d_model = 512
    d_k = 64
    d_v = 64
    n_heads = 8
    ...
    train_split = .95
    test_trials = 10
    num_epochs = 2001 # 24000 * 7
    random_seed = 10
    lr_init = 1. # using lr_scheduler
    warmup_steps = 4000

    checkpoint_path = 'ckpts_clip_vit/latest.pt' # path to a saved checkpoint (model state, optimizer state, scheduler state) to resume training from
    ...
    image_encoder = init_vit(patch_size, patch_dim, seq_len_vit, d_model, d_k, d_v, n_heads, n_layers, d_ff, dropout, out_dim, device).to(device)
    ...
    for ep in tqdm(range(num_epochs)):
        ...
        imgs, cap_tokens, cap_pad_mask, cap_causal_mask = process_batch(minibatch, model, spm_processor, sos_token, eos_token, pad_token, max_seq_len, img_size, device) # imgs.shape:[batch_size, 3, 64, 64], captions.shape:[batch_size, max_seq_len]

        # calculate loss
        loss = calculate_loss(model, imgs, cap_tokens, cap_pad_mask, cap_causal_mask, batch_size, device)

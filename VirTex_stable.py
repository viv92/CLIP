...
## Todos / Questions:
# 1. does multiplying embeddings with math.sqrt(d_model) create mismatch when inverting the output to embeddings in the decoder head?
...
# 7. The same token embedding matrix is used at the input and the output of both of the transformer decoders.
...
class CaptionEmbeddings(nn.Module):
    def __init__(self, vocab_size, max_seq_len, d_model, dropout, device):
        ...
        self.device = device
    # def build_positional_encodings(self, max_seq_len, batch_size):
    #     positions = torch.arange(max_seq_len)
    #     positions = positions.unsqueeze(0)
    #     positions = positions.expand(batch_size, -1)
    #     pos_enc = self.pos_emb(positions).to(self.device) # pos_enc.shape: [batch_size, max_seq_len, d_model]
    #     return pos_enc
    def forward(self, x):
...
class VirTex(nn.Module):
    ...
    def forward(self, src, src_mask, tgt_fwd, tgt_rev, tgt_mask):
        ...
        scores_fwd = self.projection(decoder_out_fwd) # scores.shape: [batch_size, seq_len, tgt_vocab]
        scores_rev = self.projection(decoder_out_rev)

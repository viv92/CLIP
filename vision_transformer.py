...
class ImageEmbeddings(nn.Module):
    ...
    def forward(self, x): # s.shape: [batch_size, seq_len, patch_dim]
        ...
        patch_emb = torch.cat((patch_emb, cls_emb), dim=1) # patch_emb.shape: [batch_size, seq_len+1, d_model]
...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, key, query, value, mask_pad=None, mask_causal=None): # can be used for both (unmasked) encoder attention and (masked) decoder attention
        attn_output, attn_weights = self.mha(query, key, value, key_padding_mask=mask_pad, attn_mask=mask_causal)
        return attn_output
...
class EncoderLayer(nn.Module):
    ...
    def forward(self, x, mask_pad, mask_causal):
        x = self.sublayers[0](x, lambda x: self.self_attn(x, x, x, mask_pad=mask_pad, mask_causal=mask_causal)) # x.shape: [batch_size, seq_len, d_model]
...
class Encoder(nn.Module):
    ...
    def forward(self, x, mask_pad=None, mask_causal=None):
        for layer in self.layers:
            x = layer(x, mask_pad, mask_causal)
        return self.norm(x)
